...
return new class extends Migration
{
    ...
    {
        if (
            ! Schema::hasTable('legal_compliance_templates')
        ...
        $template = DB::table('legal_compliance_templates')
            ->where('slug', 'free-legal-compliance-assessment')
            ->first()
            ?: DB::table('legal_compliance_templates')
                ->where('is_default', true)
                ->orWhere('slug', 'osh-legal-compliance-assessment')
    ...
    {
        // Keep seeded legal template data in place; assessment records may reference the version snapshot.
    }
