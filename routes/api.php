...
Route::middleware('auth.session')->group(function () {
    ...
    Route::patch('vendor-payments/{id}/approve',         [VendorController::class, 'approvePayment'])->middleware('role:Manager,System Admin');
    ...
    Route::patch('tasks/{id}/complete',                  function (\Illuminate\Http\Request $request, TaskController $controller, int $id) {
    ...
    Route::post('tasks/{id}/comments',                   function (\Illuminate\Http\Request $request, TaskController $controller, int $id) {
        $request->merge(['task_id' => $id]);
        return $controller->createComment($request);
    });
    Route::delete('tasks/{id}',                          function (\Illuminate\Http\Request $request, TaskController $controller, int $id) {
        $request->merge(['task_id' => $id]);
    ...
    Route::patch('quote-price-exceptions/{id}/approve',         [QuotePriceExceptionController::class, 'approve'])->middleware('role:Manager,System Admin');
    Route::patch('quote-price-exceptions/{id}/reject',          [QuotePriceExceptionController::class, 'reject'])->middleware('role:Manager,System Admin');
    ...
    Route::put('projects/{id}/vendors/{assignmentId}',          function (\Illuminate\Http\Request $request, ProjectController $c, int $id, int $assignmentId) {
        $request->merge(['project_id' => $id, 'assignment_id' => $assignmentId]);
        return $c->updateVendor($request);
    });
    Route::delete('projects/{id}/vendors/{assignmentId}',       function (\Illuminate\Http\Request $request, ProjectController $c, int $id, int $assignmentId) {
        $request->merge(['project_id' => $id, 'assignment_id' => $assignmentId]);
